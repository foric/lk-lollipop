...
int kdump_ext4_output(struct mrdump_control_block *mrdump_cb, uint32_t total_dump_size, struct mrdump_dev *mrdump_dev)
{
    ...
    if(kparams->output_lbaooo == 0) {
        voprintf_error(" No_Delete.rdmp has no LBA markers. RAM-Dump stop!\n");
        return -1;
    }
    ...
    voprintf_info(" SYS_COREDUMP File Size is : %u\n", handle->filesize);
    ...
    if (ok) {
        ...
        mrdump_status_ok("%s:%s\nPART_USER-OUTPUT\nFILENAME:%s\n", mrdump_dev->name, "ext4", "FRDump.zip");
    }
