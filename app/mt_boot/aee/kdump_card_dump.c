...
#ifdef MTK_NEW_COMBO_EMMC_SUPPORT
#ifdef MTK_GPT_SCHEME_SUPPORT
    bool retval = emmc_write(part->part_id, (part->start_sect * BLK_SIZE) + offset, (unsigned char *)buf, len) == len;
#else
    bool retval = emmc_write(part->part_id, (part->startblk * BLK_SIZE) + offset, (unsigned char *)buf, len) == len;
#endif
#else
#ifdef MTK_GPT_SCHEME_SUPPORT
    bool retval = emmc_write((part->start_sect * BLK_SIZE) + offset, (unsigned char *)buf, len) == len;
#else
