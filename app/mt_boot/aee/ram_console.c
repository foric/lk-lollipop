...
{
    unsigned int fiq_step, wdt_status;
    if (ram_console && ram_console->off_linux) {
	wdt_status = *(u32*)((void*)ram_console + ram_console->off_pl);
	fiq_step = *(u32*)((void*)ram_console + ram_console->off_linux);
	printf("%s. wdt_status 0x%x, fiq_step 0x%x\n", MOD, wdt_status, fiq_step);
