...
static uint8_t *kdump_core_write_cpu_note(const struct mrdump_control_block *mrdump_cb, int cpu, struct elf64_phdr *nhdr, uint8_t *bufp)
{
    ...
    memcpy(&prstatus.pr_reg, &mrdump_cb->crash_record.arm64_regs[cpu], sizeof(elf_gregset_t));
...
void *kdump_core64_header_init(const struct mrdump_control_block *mrdump_cb, uint64_t kmem_address, uint64_t kmem_size)
{
	...
	elf->e_entry	= 0;
	elf->e_phoff	= sizeof(struct elf64_hdr);
	elf->e_shoff	= 0;
	elf->e_flags	= ELF_CORE_EFLAGS;
	elf->e_ehsize	= sizeof(struct elf64_hdr);
	elf->e_phentsize= sizeof(struct elf64_phdr);
	elf->e_phnum	= 2;
	elf->e_shentsize= 0;
	elf->e_shnum	= 0;
	elf->e_shstrndx	= 0;
