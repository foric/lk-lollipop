...
void cmd_boot(const char *arg, void *data, unsigned sz)
{
  ...
  printf("============================================================\n");
   ...
   else
   {
    ...
    if((hdr.kernel_addr <= ROUND_TO_PAGE((hdr.ramdisk_addr + hdr.ramdisk_size), page_mask)) && (ROUND_TO_PAGE((hdr.kernel_addr + hdr.kernel_size), page_mask) >= hdr.ramdisk_addr)) {
		fastboot_fail("invalid kernel & ramdisk address: images overlap");
		return;
    }

    if((hdr.kernel_addr < DRAM_PHY_ADDR) || (ROUND_TO_PAGE((hdr.kernel_addr + hdr.kernel_size), page_mask) > (DRAM_PHY_ADDR + memory_size()))) {
		fastboot_fail("invalid kernel address: not lie in memory");
		return;
    }

    if((hdr.kernel_addr <= _heap_end) && (ROUND_TO_PAGE((hdr.kernel_addr + hdr.kernel_size), page_mask) >= MEMBASE)) {
		fastboot_fail("invalid kernel address: overlap with lk");
		return;
    }

    if((hdr.kernel_addr <= ROUND_TO_PAGE((SCRATCH_ADDR + hdr.kernel_size + hdr.ramdisk_size), page_mask)) && (ROUND_TO_PAGE((hdr.kernel_addr + hdr.kernel_size), page_mask) >= SCRATCH_ADDR)) {
		fastboot_fail("invalid kernel address: overlap with the download image");
		return;
    }

    if((hdr.ramdisk_addr < DRAM_PHY_ADDR) || (ROUND_TO_PAGE((hdr.ramdisk_addr + hdr.ramdisk_size), page_mask) > (DRAM_PHY_ADDR + memory_size()))) {
		fastboot_fail("invalid ramdisk address: not lie in memory");
		return;
    }

    if((hdr.ramdisk_addr <= _heap_end) && (ROUND_TO_PAGE((hdr.ramdisk_addr + hdr.ramdisk_size), page_mask) >= MEMBASE)) {
		fastboot_fail("invalid ramdisk address: overlap with lk");
		return;
    }

    if((hdr.ramdisk_addr <= ROUND_TO_PAGE((SCRATCH_ADDR + hdr.kernel_size + hdr.ramdisk_size), page_mask)) && (ROUND_TO_PAGE((hdr.ramdisk_addr + hdr.ramdisk_size), page_mask) >= SCRATCH_ADDR)) {
    ...
    if (mboot_android_check_img_info(PART_KERNEL, (part_hdr_t *)(ptr + hdr.page_size)) == -1) {
        printf("no mkimg header in kernel image\n");
        memmove((void*) hdr.kernel_addr, (ptr + hdr.page_size), hdr.kernel_size);
    }
    else {
        printf("mkimg header exist in kernel image\n");
        memmove((void*) hdr.kernel_addr, (ptr + hdr.page_size + MKIMG_HEADER_SZ), hdr.kernel_size);
    }

    if ((mboot_android_check_img_info(PART_ROOTFS, (part_hdr_t *)(ptr + hdr.page_size + kernel_actual)) == -1) 
        && (mboot_android_check_img_info("RECOVERY", (part_hdr_t *)(ptr + hdr.page_size + kernel_actual)) == -1)) {
        printf("no mkimg header in ramdisk image\n");
        memmove((void*) hdr.ramdisk_addr, (ptr + hdr.page_size + kernel_actual), hdr.ramdisk_size);
    }
    else {
        printf("mkimg header exist in ramdisk image\n");
        memmove((void*) hdr.ramdisk_addr, (ptr + hdr.page_size + kernel_actual + MKIMG_HEADER_SZ), hdr.ramdisk_size);
    }

#ifndef MACH_FPGA
    sprintf(g_CMDLINE, "%s %s", g_CMDLINE, hdr.cmdline);
