...
static void DSI_PHY_clk_adjusting(void)
{
	...
	if(!has_adjusted){
		...
		DSI_OUTREG32_R(PMIPITX_DSI_BG_CON_REG,&DSI_PHY_REG->MIPITX_DSI_BG_CON, &temp_BG_CON_REG);
...
//		pmod = 433.33;
	...
	DSI_OUTREGBIT(MIPITX_DSI0_CLOCK_LANE_REG,DSI_PHY_REG->MIPITX_DSI0_CLOCK_LANE,RG_DSI0_LNTC_RT_CODE,0x8);
	...
	{
		DSI_OUTREGBIT(MIPITX_DSI0_DATA_LANE0_REG,DSI_PHY_REG->MIPITX_DSI0_DATA_LANE0,RG_DSI0_LNT0_RT_CODE,0x8);
	...
	{
		DSI_OUTREGBIT(MIPITX_DSI0_DATA_LANE1_REG,DSI_PHY_REG->MIPITX_DSI0_DATA_LANE1,RG_DSI0_LNT1_RT_CODE,0x8);
	...
	{
		DSI_OUTREGBIT(MIPITX_DSI0_DATA_LANE2_REG,DSI_PHY_REG->MIPITX_DSI0_DATA_LANE2,RG_DSI0_LNT2_RT_CODE,0x8);
	...
	{
		DSI_OUTREGBIT(MIPITX_DSI0_DATA_LANE3_REG,DSI_PHY_REG->MIPITX_DSI0_DATA_LANE3,RG_DSI0_LNT3_RT_CODE,0x8);
...
//#define DSI_ENABLE_PWM_FOR_TE_LK
...
static struct pwm_spec_config pwm_setting = {
		...
		.pmic_pad = false,
		.pwm_mode.PWM_MODE_OLD_REGS.IDLE_VALUE = IDLE_FALSE,
		.pwm_mode.PWM_MODE_OLD_REGS.GUARD_VALUE = 0, /* in old mode, this value is invalid */
		.pwm_mode.PWM_MODE_OLD_REGS.GDURATION = 0,
		.pwm_mode.PWM_MODE_OLD_REGS.WAVE_NUM = 0,                /* 0 == none stop until the PWM is disable */    
           /* 135 : 60.24HZ, margin btw touch_en & pwm_falling_edge=740uS */
		.clk_div = CLK_DIV4,            
		.pwm_mode.PWM_MODE_OLD_REGS.DATA_WIDTH = 138,//58.2fps //135 //60.2fps
		.pwm_mode.PWM_MODE_OLD_REGS.THRESH =138/2	
};

DSI_STATUS DSI_SetPWMForMixMode(unsigned int fps)
{    
    unsigned int width = 32 * 1024 / (4 * fps) - 1;
    
    printf("DSI_SetPWMForMixMode \n");

    pwm_setting.pwm_mode.PWM_MODE_OLD_REGS.DATA_WIDTH = width;
    pwm_setting.pwm_mode.PWM_MODE_OLD_REGS.THRESH = width/2;
...
DSI_STATUS DSI_SetMixMode(unsigned int clk)
{
    DSI_T0_INS t0;
        
    printf("DSI_SetMixMode clk (%d) \n", clk);
...
DSI_STATUS DSI_StartTransfer(BOOL needStartDSI)
{
	...
	{
#ifdef CONFIG_MIXMODE_FOR_INCELL
#ifdef DSI_ENABLE_PWM_FOR_TE_LK
        if (lcm_params->dsi.mixmode_enable)
        {
    		DSI_SetPWMForMixMode(lcm_params->dsi.pwm_fps);
    		DSI_SetMixMode(234); // don't use lcm_params->dsi.mixmode_mipi_clock
        }
#endif        
#endif
