...
#define MMC_TST_SBLK_RW      (0)
#define MMC_TST_MBLK_RW      (1)
#define MMC_TST_IMBLK_RW     (0)
...
//#define MMC_TST_CHUNK_BLKS   (512)              /* 64KB */
static unsigned int mmc_tst_size[] = { 16 * 512 * 10 ,32 * 512 * 10,64 * 512 * 10,128 * 512 * 10,256 * 512 * 10,512 * 512 * 10,1024 * 512 * 10};
static unsigned int mmc_tst_chunk_blks[] = { 16,32,64,128,256,512,1024};
#else
...
static unsigned int buswidth[] = { HOST_BUS_WIDTH_8 };
char buff[1024 * 512 * 10*2];
...
void mmc_prof_update(mmc_prof_callback cb, unsigned long id, void *data)
{
    unsigned long counts = (unsigned long)msdc_timer_get_count();
    if (cb) {
        cb(data, id, counts);
    }
}
...
int mmc_erase_blks(int dev_id, u64 addr, u32 size, int bootarea)
{
    unsigned long ret;
    int i, j, result = 0;
    u8 val;
    ...
    if (!result) {
        printf("[SD%d] Erase %lld blocks (%lld bytes) from 0x%x successfully\n", 
            dev_id, total_blks, total_blks * MMC_BLOCK_SIZE, blknr * MMC_BLOCK_SIZE);
    } else {
        printf("[SD%d] Erase %lld blocks (%lld bytes) from 0x%x failed %d\n", 
            dev_id, total_blks, total_blks * MMC_BLOCK_SIZE, blknr * MMC_BLOCK_SIZE, result);
    }    
    return result;
}
...
int mmc_test_mem_card(struct mmc_test_config *cfg)
{
    ...
    while (forever || count--) {
        ...
        if (mmc_erase_start(card, (u64)(blknr * blksz)) != MMC_ERR_NONE) {
            result = -__LINE__;
            goto failure;
        }
        if (mmc_erase_end(card, (u64)((blknr + total_blks) * blksz)) != MMC_ERR_NONE) {
            result = -__LINE__;
            goto failure;
        }
        if (mmc_erase(card, MMC_ERASE_NORMAL) != MMC_ERR_NONE) {
...
int mmc_test(int argc, char *argv[])
{
    ...
    for (i = 0; i < ARRAY_SIZE(clkfreq); i++) {	
        for (j = 0; j < ARRAY_SIZE(buswidth); j++) {
			for(k = 0; k < ARRAY_SIZE(mmc_tst_chunk_blks);k++){
            cfg.clock = clkfreq[i];
            cfg.buswidth = buswidth[j];
			cfg.chunk_blks = mmc_tst_chunk_blks[k];
			cfg.total_size = mmc_tst_size[k];
